...
{
    public partial class B_SalaryBasic11 : System.Web.UI.Page
    {
        ...
        private bool boolCHK_ID_Data(string strID_Data)
        {
            using (SqlConnection SelectConn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
            {
                ...
                try
                {
                    string strSQL = "SELECT * FROM B_BOSS WHERE \"ID\" = '" + strID_Data + "'";
        ...
        protected void btCreate_Click(object sender, EventArgs e)
        {
            ...
            if (boolCHK_ID_Data(strID) == false)
            {
                using (SqlConnection insertconn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
                {
                    ...
                    try
                    {
                        string insertString = "Insert Into B_BOSS (\"ID\",\"NAME\",PAY,CREATEDATE) "
        ...
        protected void btUpdate_Click(object sender, EventArgs e)
        {
            ...
            //    string UpdateString = "UPDATE B_BOSS SET OLD_DATA=@OLD_DATA "
            //        + " WHERE B_SNID = '" + Session["B_BOSSSelectKeyword"] + "' ";
        ...
        protected void GridView_BOSS_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            //    string strKeyword = GridView_BOSS.DataKeys[index].Value.ToString();

            //    Session["B_BOSSSelectKeyword"] = strKeyword;

            //    using (SqlConnection Selectconn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
            //    {
            //        Selectconn.Open();

            //        string strSQL = "SELECT * FROM B_BOSS WHERE B_SNID = '" + Session["B_BOSSSelectKeyword"] + "' ";
            //        SqlCommand Selectcmd = new SqlCommand(strSQL, Selectconn);
