...
{
    public partial class B_SalaryBasic5 : System.Web.UI.Page
    {
        ...
        private bool boolCHK_ID_Data(string strID_Data)
        {
            using (SqlConnection SelectConn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
            {
                ...
                try
                {
                    string strSQL = "SELECT * FROM B_TECHNICS WHERE \"ID\" = '" + strID_Data + "'";
        ...
        protected void btCreate_Click(object sender, EventArgs e)
        {
            ...
            if (boolCHK_ID_Data(strID) == false)
            {
                using (SqlConnection insertconn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
                {
                    ...
                    try
                    {
                        string insertString = "Insert Into B_TECHNICS (\"ID\",\"NAME\",PAY,CREATEDATE) "
        ...
        protected void btUpdate_Click(object sender, EventArgs e)
        {
            ...
            //    string UpdateString = "UPDATE B_TECHNICS SET OLD_DATA=@OLD_DATA "
            //        + " WHERE T_SNID = '" + Session["B_TECHNICSSelectKeyword"] + "' ";
        ...
        protected void GridView_TECHNICS_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            //    string strKeyword = GridView_TECHNICS.DataKeys[index].Value.ToString();

            //    Session["B_TECHNICSSelectKeyword"] = strKeyword;

            //    using (SqlConnection Selectconn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
            //    {
            //        Selectconn.Open();

            //        string strSQL = "SELECT * FROM B_TECHNICS WHERE T_SNID = '" + Session["B_TECHNICSSelectKeyword"] + "' ";
            //        SqlCommand Selectcmd = new SqlCommand(strSQL, Selectconn);
