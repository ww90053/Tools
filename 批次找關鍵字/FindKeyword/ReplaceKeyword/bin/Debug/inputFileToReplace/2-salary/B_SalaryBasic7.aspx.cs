...
{
    public partial class B_SalaryBasic7 : System.Web.UI.Page
    {
        ...
        private bool boolCHK_ID_Data(string strID_Data)
        {
            using (SqlConnection SelectConn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
            {
                ...
                try
                {
                    string strSQL = "SELECT * FROM B_BONUS WHERE \"ID\" = '" + strID_Data + "'";
        ...
        protected void btCreate_Click(object sender, EventArgs e)
        {
            ...
            if (boolCHK_ID_Data(strID) == false)
            {
                using (SqlConnection insertconn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
                {
                    ...
                    try
                    {
                        string insertString = "Insert Into B_BONUS (\"ID\",\"NAME\",PAY,CREATEDATE) "
        ...
        protected void btUpdate_Click(object sender, EventArgs e)
        {
            ...
            //    string UpdateString = "UPDATE B_BONUS SET OLD_DATA=@OLD_DATA "
            //        + " WHERE B_SNID = '" + Session["B_BONUSSelectKeyword"] + "' ";
        ...
        protected void GridView_BONUS_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            //    string strKeyword = GridView_BONUS.DataKeys[index].Value.ToString();

            //    Session["B_BONUSSelectKeyword"] = strKeyword;

            //    using (SqlConnection Selectconn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
            //    {
            //        Selectconn.Open();

            //        string strSQL = "SELECT * FROM B_BONUS WHERE B_SNID = '" + Session["B_BONUSSelectKeyword"] + "' ";
            //        SqlCommand Selectcmd = new SqlCommand(strSQL, Selectconn);
