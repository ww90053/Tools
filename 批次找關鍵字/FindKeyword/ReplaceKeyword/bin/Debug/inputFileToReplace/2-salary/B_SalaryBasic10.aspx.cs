...
{
    public partial class B_SalaryBasic10 : System.Web.UI.Page
    {
        ...
        private bool boolCHK_ID_Data(string strID_Data)
        {
            using (SqlConnection SelectConn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
            {
                ...
                try
                {
                    string strSQL = "SELECT * FROM B_PROFESS WHERE \"ID\" = '" + strID_Data + "'";
        ...
        protected void btCreate_Click(object sender, EventArgs e)
        {
            ...
            Label_MSG.Text = "";
                using (SqlConnection insertconn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
                {
                    ...
                    try
                    {
                        string insertString = "Insert Into B_PROFESS (\"ID\",\"NAME\",PAY,CREATEDATE) "
        ...
        protected void btUpdate_Click(object sender, EventArgs e)
        {
            ...
            //    string UpdateString = "UPDATE B_PROFESS SET OLD_DATA=@OLD_DATA "
            //        + " WHERE P_SNID = '" + Session["B_PROFESSSelectKeyword"] + "' ";
        ...
        protected void GridView_PROFESS_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            //    string strKeyword = GridView_PROFESS.DataKeys[index].Value.ToString();

            //    Session["B_PROFESSSelectKeyword"] = strKeyword;

            //    using (SqlConnection Selectconn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
            //    {
            //        Selectconn.Open();

            //        string strSQL = "SELECT * FROM B_PROFESS WHERE P_SNID = '" + Session["B_PROFESSSelectKeyword"] + "' ";
            //        SqlCommand Selectcmd = new SqlCommand(strSQL, Selectconn);
