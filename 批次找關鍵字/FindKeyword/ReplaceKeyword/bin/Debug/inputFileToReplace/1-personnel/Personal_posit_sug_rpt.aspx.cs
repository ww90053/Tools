...
{
    public partial class Personal_posit_sug_rpt : System.Web.UI.Page
    {
        int TPM_FION=0;
...
//                            FROM A_POSIT2 
//                            WHERE MZ_PRID='" + DropDownList_MZ_PRID.SelectedItem.Text + "' AND MZ_PRID1='" + o_str.tosql(TextBox_MZ_PRID1.Text.Trim()) + "'";
//            //" OR " +
            //            //" (MZ_BPRID='" + DropDownList_MZ_PRID.SelectedItem.Text + "' AND MZ_BPRID1='" + o_str.tosql(TextBox_MZ_PRID1.Text.Trim()) + "'))";
            #region SQL 
            string strSQL = @"SELECT MZ_ID	,MZ_NAME	,A_POSIT2.MZ_PRID	,MZ_PRID1	,MZ_EXOPOS
	,MZ_NREA	,AKNR.MZ_KCHI MZ_NREA_NAME	,MZ_EXOAD	,AKEXOD.MZ_KCHI MZ_EXOAD_NAME	,MZ_EXOUNIT	,AKEXOU.MZ_KCHI MZ_EXOUNIT_NAME
	,MZ_EXOOCC	,AKEXOO.MZ_KCHI MZ_EXOOCC_NAME	,MZ_EXORANK	,AKEXOR.MZ_KCHI MZ_EXORANK_NAME	,MZ_EXRANK1	,AKEXOR1.MZ_KCHI MZ_EXRANK1_NAME
	,MZ_SPT
	,MZ_REMARK
	,OTH_THING
	,CONDITION
	...
	,MZ_EXOPNO1
	,A_POSIT2.MZ_AD
	,AKD.MZ_KCHI MZ_AD_NAME
	,MZ_UNIT
	,AKU.MZ_KCHI MZ_UNIT_NAME
	,MZ_OCCC
	,AKO.MZ_KCHI MZ_OCCC_NAME
	,MZ_RANK
	,AKR.MZ_KCHI MZ_RANK_NAME
	,MZ_RANK1
	,AKR1.MZ_KCHI MZ_RANK1_NAME
	,MZ_SRANK
	,AKSR.MZ_KCHI MZ_SRANK_NAME
	,MZ_CHISI
	,AKCH.MZ_KCHI MZ_CHISI_NAME
	,MZ_SLVC
	,AKSL.MZ_KCHI MZ_SLVC_NAME
	,(CONVERT(VARCHAR(200),dbo.to_number(dbo.SUBSTR(MZ_DATE, 0, 3))) + '年' + dbo.SUBSTR(MZ_DATE, 4, 2) + '月' + dbo.SUBSTR(MZ_DATE, 6, 2) + '日') AS MZ_DATE
	,(CONVERT(VARCHAR(200),dbo.to_number(dbo.SUBSTR(MZ_IDATE, 0, 3))) + '年' + dbo.SUBSTR(MZ_IDATE, 4, 2) + '月' + dbo.SUBSTR(MZ_IDATE, 6, 2) + '日') AS MZ_IDATE
	,ACK.SPEED_NO SPEED_NO
	,ACK.PWD_NO PWD_NO
FROM A_POSIT2
...
LEFT JOIN A_CHKAD ACK ON ACK.MZ_PRID = A_POSIT2.MZ_PRID	AND ACK.MZ_AD = A_POSIT2.MZ_CHKAD
            ...
            for (int i = 0; i < tempDT.Rows.Count; i++)
            {
                ...
                //20140123
                //newdr["COUNTMAN"] = DateChange.toChinese o_crystal.toChinese((i + 1).ToString(), 1) + "、";
            ...
            strSQL = string.Format("SELECT MZ_EXPLAIN,MZ_EXPLAIN0,MZ_EXPLAIN1 FROM A_POSIT1 WHERE MZ_PRID='{0}' AND MZ_PRID1='{1}'", DropDownList_MZ_PRID.SelectedItem.Text, TextBox_MZ_PRID1.Text);
            ...
            strSQL = string.Format("SELECT MZ_MASTER_NAME,MZ_MASTER_POS FROM A_CHKAD WHERE MZ_PRID='{0}'", DropDownList_MZ_PRID.SelectedItem.Text);
