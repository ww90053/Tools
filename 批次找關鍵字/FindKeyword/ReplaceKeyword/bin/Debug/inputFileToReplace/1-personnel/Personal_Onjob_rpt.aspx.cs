...
{
    public partial class A_Onjob_rpt : System.Web.UI.Page
    {
        int TPM_FION=0;
        //string A_strGID = "";
        ...
        protected void btPrint_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Onjob.Rows.Count; i++)
            {
                ...
                Onjob.Rows[i]["PAY1"] = o_DBFactory.ABC_toTest.vExecSQL("SELECT MZ_SPT FROM A_DLBASE WHERE MZ_ID='" + Onjob.Rows[i]["MZ_ID"].ToString() + "'");
            ...
            Session["TITLE"] = string.Format("{0}員警就職傳知名冊", o_A_KTYPE.RAD(Session["ADPMZ_EXAD"].ToString()));

            Session["rpt_dt"] = Onjob;

            string tmp_url = "A_rpt.aspx?fn=Onjob&TPM_FION=" + TPM_FION;
