...
{
    public partial class C_ForLeaveOvertime_ONDUTY_CHECK : System.Web.UI.Page
    {
        ...
        public void doSearch(string EXAD, string EXUNIT, string DATE1, string DATE2, string MZ_NAME, string MZ_ID, string IS_CHK)
        {
            ...
            string gvSelectCommand = @"SELECT CC.SN,CC.MZ_ID ,MZ_NAME,MZ_OCCC_CH, DATE_TAG ,C1.DUTY_KINDNAME ,  C2.DUTY_KINDNAME HOURTYPE, PAY , dbo.TO_CHAR(TIME_SATRT,'yyyy/MM/dd HH24:mi' )+ '~ '+ dbo.TO_CHAR(TIME_END,'yyyy/MM/dd HH24:mi' ) REAL_DATE ,
                                           ...
                                           CASE IS_CHK_UNIT WHEN 'Y' THEN '已審核' ELSE '未審核' END IS_CHK_UNIT ,  IS_CHK ,MEMO
            ...
            {
                    gvSelectCommand += "AND IS_CHK='" + IS_CHK + "' ";
        ...
        protected void Button_Check_Click(object sender, EventArgs e)
        {
            ...
            string Updatesql_Y = "UPDATE C_ONDUTY_DAY SET IS_CHK=@IS_CHK,CHKUSER=@CHKUSER , CHKDATE=@CHKDATE WHERE SN IN ({0})";
            string Updatesql_N = "UPDATE C_ONDUTY_DAY SET IS_CHK=@IS_CHK,CHKUSER=@CHKUSER , CHKDATE=@CHKDATE WHERE SN IN ({0})";
            ...
            parY.Add(new SqlParameter("IS_CHK", SqlDbType.VarChar) { Value = "Y" });
            parY.Add(new SqlParameter("CHKDATE", SqlDbType.DateTime) { Value = DateTime.Now });
            parY.Add(new SqlParameter("CHKUSER", SqlDbType.VarChar) { Value = Session["ADPMZ_ID"].ToString() });

            List<SqlParameter> parN = new List<SqlParameter>();
            //parN.Add(new SqlParameter("SN", SqlDbType.Float) { Value = NN });
            parN.Add(new SqlParameter("IS_CHK", SqlDbType.VarChar) { Value = "N" });
            parN.Add(new SqlParameter("CHKDATE", SqlDbType.DateTime) { Value = DateTime.Now });
            parN.Add(new SqlParameter("CHKUSER", SqlDbType.VarChar) { Value = Session["ADPMZ_ID"].ToString() });
