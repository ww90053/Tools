...
{
    public partial class C_ForleaveOverTime_stat : System.Web.UI.Page
    {
        string TPM_FION = "";
        /// <summary>
        /// 初始化各頁面功能
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                PageInitial(sg);
                txt_Year.Text = (DateTime.Today.Year - 1911).ToString();
                rbl_halftype.SelectedValue = "0";
        ...
        protected void btn_statrpt1_Click(object sender, EventArgs e)
        {
            string pdflink1 = System.Configuration.ConfigurationManager.AppSettings["pdflink"].ToString();

            string tmp_url = "C_overtime_statrpt1.aspx?pdf=1&ctype=" + rbl_print.SelectedValue + "&MZ_ID=" + txt_MZ_ID.Text.Trim() + "&PM_FION=" + TPM_FION +
                     "&Year=" + txt_Year.Text + "&Yearend=" + rbl_halftype.SelectedValue +
                     "&AD=" + ddl_Search_AD.SelectedValue + "&UNIT=" + ddl_Search_Unit.SelectedValue;
            //string filename = "/userfiles/" + DateTime.Now.ToString("yyyyMMddHHmmssff") + ".pdf";
            string url = "http://" + Request.ServerVariables["Server_Name"] + ":" + Request.ServerVariables["Server_port"] + "/3-forleave/" + tmp_url;
            //System.Diagnostics.Process.Start(pdflink1, " -s A4 -L 0 -R 0 -T 2 -B 0 " + url + " " + Server.MapPath(filename));
        ...
        protected void btn_statrpt2_Click(object sender, EventArgs e)
        {
            string pdflink1 = System.Configuration.ConfigurationManager.AppSettings["pdflink"].ToString();

            string tmp_url = "C_overtime_statrpt2.aspx?pdf=0&ctype=" + rbl_print.SelectedValue + "&MZ_ID=" + txt_MZ_ID.Text.Trim() + "&PM_FION=" + TPM_FION +
                     "&Year=" + txt_Year.Text + "&Yearend=" + rbl_halftype.SelectedValue +
                     "&AD=" + ddl_Search_AD.SelectedValue + "&UNIT=" + ddl_Search_Unit.SelectedValue;
            // string filename = "/userfiles/" + DateTime.Now.ToString("yyyyMMddHHmmssff") + ".pdf";
            string url = "http://" + Request.ServerVariables["Server_Name"] + ":" + Request.ServerVariables["Server_port"] + "/3-forleave/" + tmp_url;
