...
{
    public partial class C_ForLeaveOvertime_DUTYCHECK : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            C.set_Panel_EnterToTAB(ref this.Panel1);
                ...
                if (ad_unit.Rows.Count > 0)
                    Label1.Text = ad_unit.Rows[0]["MZ_EXAD"].ToString() + ad_unit.Rows[0]["MZ_EXUNIT"].ToString() + "巡邏勤務盤查點設定";
        ...
        protected void execSQL()
        {
            ...
            string old_DUTYCHECK_NO = "NULL";

            if (ViewState["Mode"].ToString() == "UPDATE")
            {
                HttpCookie Cookie1 = new HttpCookie("PKEY_DUTYCHECK_NO");
                Cookie1 = Request.Cookies["PKEY_DUTYCHECK_NO"];
                old_DUTYCHECK_NO = Cookie1.Value.ToString();
            }

            string pkey_check;

            if (old_DUTYCHECK_NO == TextBox_DUTYCHECK_NO.Text.Trim() && ViewState["Mode"].ToString() == "UPDATE")
                pkey_check = "0";
            else
                pkey_check = o_DBFactory.ABC_toTest.vExecSQL("SELECT COUNT(*) FROM C_DUTYCHECK WHERE DUTYCHECK_NO='" + TextBox_DUTYCHECK_NO.Text.Trim() + "' AND MZ_AD='" + Session["ADPMZ_EXAD"].ToString() + "'AND MZ_UNIT='" + Session["ADPMZ_EXUNIT"].ToString() + "'");

            if (pkey_check != "0")
            {
                ErrorString += "巡邏勤務盤查點編號違反唯一值條件。" + "\\r\\n";
                TextBox_DUTYCHECK_NO.BackColor = Color.Orange;
            }
            else
            {
                TextBox_DUTYCHECK_NO.BackColor = Color.White;
            }
            ...
            //        string result = o_Check_Dbdic.ErrorString(tbox.ID, "C_DUTYCHECK", tbox.Text);
            ...
            using (SqlConnection conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["MSDBConnectionString_toTest"].ConnectionString))
            {
                ...
                try
                {
                    ...
                    cmd.Parameters.Add("DUTYCHECK_NO", SqlDbType.VarChar).Value = TextBox_DUTYCHECK_NO.Text.Trim().ToUpper();
                    cmd.Parameters.Add("DUTYCHECK", SqlDbType.VarChar).Value = TextBox_DUTYCHECK.Text.Trim().ToUpper();
                    cmd.ExecuteNonQuery();

                    Response.Cookies["PKEY_DUTYCHECK_NO"].Expires = DateTime.Now.AddYears(-1);
                    ...
                    TextBox_DUTYCHECK.Text = string.Empty;
                    TextBox_DUTYCHECK_NO.Text = string.Empty;
        ...
        protected void btInsert_Click(object sender, EventArgs e)
        {
            ...
            ViewState["CMDSQL"] = "INSERT INTO " +
                                               "C_DUTYCHECK(MZ_AD,MZ_UNIT,DUTYCHECK_NO,DUTYCHECK) " +
                                  " VALUES " +
                                               "(@MZ_AD,@MZ_UNIT,@DUTYCHECK_NO,@DUTYCHECK)";
            ...
            ScriptManager.RegisterStartupScript(this.UpdatePanel1, typeof(string), "", "$get('" + TextBox_DUTYCHECK_NO.ClientID + "').focus();$get('" + TextBox_DUTYCHECK_NO.ClientID + "').focus();", true);
        }
        ...
        protected void btUpdate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ViewState["CMDSQL"] = "UPDATE " +
                                                 "C_DUTYCHECK " +
                                      "SET " +
                                                 "MZ_AD=@MZ_AD,MZ_UNIT=@MZ_UNIT,DUTYCHECK_NO=@DUTYCHECK_NO,DUTYCHECK=@DUTYCHECK" +
                                      " WHERE " +
                                                 "  MZ_AD='" + Session["ADPMZ_EXAD"].ToString() +
                                                 "' AND MZ_UNIT='" + Session["ADPMZ_EXUNIT"].ToString() +
                                                 "' AND DUTYCHECK_NO='" + GridView1.SelectedRow.Cells[0].Text.Trim() + "'";
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Select")
            {
                ...
                TextBox_DUTYCHECK_NO.Text = GridView1.SelectedRow.Cells[0].Text.Trim();
                TextBox_DUTYCHECK.Text = GridView1.SelectedRow.Cells[1].Text.Trim();
            }
            C.controlEnable(ref this.Panel1, true);
        }
