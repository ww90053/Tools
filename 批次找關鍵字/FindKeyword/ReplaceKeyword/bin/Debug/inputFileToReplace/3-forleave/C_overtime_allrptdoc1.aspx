<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="C_overtime_allrptdoc1.aspx.cs" Inherits="TPPDDB._3_forleave.C_overtime_allrptdoc1" %>
<% string textfont = "";
    if (Request["pdf"] != "1")
    {
      string filename = HttpContext.Current.Server.UrlEncode("超勤加班費補發總清冊.doc");
...
<body>
    <center>
                 <%
                     ...
                     string strSQL = "SELECT distinct MZ_AD, MZ_UNIT, ";
                     //0:依照編制單位,1:依照現福單位
                        if (type1 == "0")
                         {
                         ...
                         {
                              strSQL = "SELECT distinct MZ_EXAD as MZ_AD, MZ_EXUNIT as MZ_UNIT, ";
                         }
                     ...
                     if (totalnum != "")
                     {
                         quotient = int.Parse(totalnum) / 15; //頁數
                         totalint = int.Parse(totalnum);      //筆數
                         surplus = int.Parse(totalnum) % 15;  //最後一頁的筆數
                         if (surplus > 1)
                         {
                             quotient++;
                         }else if (surplus == 0 )
                    ...
                    <table border="0" width="100%" height="1400">
	                        ...
	                        <tr height=100 style="font-size:20px;" > 
		                        <td align=center colspan="6"><strong><%=adtitle%><%=unitname%><%=year1.Substring(0,3)%>年<%=year1.Substring(3,2)%>月至<%=year2.Substring(3,2)%>月超勤加班費補發總清冊</strong></td>
	                        </tr>
	                        <tr>
		                        <td colspan="8">  
                        ...
                        <%}%>
                                        <tr height="25">
                                            ...
                                            <%
                                                ...
                                                int icount = int.Parse(totalnum);
                                                if (icount > 1)
                                                {
                                                    //如果跟上一筆資料是同一個人
                                                    if (strtmp == dr["MZ_ID"].ToString())
                                                    {
                                                        //type1 = "1";
                                                        //更改要查詢的單位,通常出現這種資料,代表該人員有調動單位
                                                        //unit = dr["MZ_EXUNIT"].ToString();
                                                    }
                                                    strtmp = dr["MZ_ID"].ToString();
                                                }
                                                for(int j = 1; j <=6; j++) {
                                                    //重新綁定一次要進行子查詢的單位代碼
                                                    unit = dr["MZ_UNIT"].ToString();
